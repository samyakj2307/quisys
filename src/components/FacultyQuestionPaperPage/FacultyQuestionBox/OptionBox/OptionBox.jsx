...
function OptionBox(props) {
  ...
  function handleOptionDelete(id) {
    setQuestions((prevQuestions) => {
      prevQuestions[propQno].options = prevQuestions[propQno].options.filter(
        (optionItem) => {
    ...
    <Container>
      ...
      <Row>
        <Col style={{ marginLeft: "70px" }}>
          <button onClick={addOption} className={" addButton addOptionButton"}>
            <img src={plusSign} alt={"Add Option"} />
          </button>
        </Col>
      </Row>
    </Container>
